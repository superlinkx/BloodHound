...
func (s Permission) ConvertToAppModel() model.Permission {
...
func (s Permissions) ConvertToAppModel() model.Permissions {
	var converted = make(model.Permissions, 0, len(s))

	for _, permission := range s {
		converted = append(converted, permission.ConvertToAppModel())
	}
...
func (s AuthToken) ConvertToAppModel() model.AuthToken {
	return model.AuthToken{
		...
		Unique:     s.Unique.ConvertToAppModel(),
	}
}

type AuthTokens []AuthToken

func (s AuthTokens) ConvertToAppModel() model.AuthTokens {
	var converted = make(model.AuthTokens, 0, len(s))

	for _, authToken := range s {
		converted = append(converted, authToken.ConvertToAppModel())
	}
...
func (s *AuthSecret) ConvertToAppModel() *model.AuthSecret {
...
func (s Role) ConvertToAppModel() model.Role {
	return model.Role{
		Name:        s.Name,
		Description: s.Description,
		Permissions: s.Permissions.ConvertToAppModel(),
		Serial:      s.Serial.ConvertToAppModel(),
	}
}
...
func (s Roles) ConvertToAppModel() model.Roles {
	var converted = make(model.Roles, 0, len(s))

	for _, role := range s {
		converted = append(converted, role.ConvertToAppModel())
	}
...
func (s *OIDCProvider) ConvertToAppModel() *model.OIDCProvider {
	return &model.OIDCProvider{
		ClientID:      s.ClientID,
		Issuer:        s.Issuer,
		SSOProviderID: s.SSOProviderID,
		Serial:        s.Serial.ConvertToAppModel(),
	}
}
...
func (s *SAMLProvider) ConvertToAppModel() *model.SAMLProvider {
	return &model.SAMLProvider{
		Name:                         s.Name,
		DisplayName:                  s.DisplayName,
		IssuerURI:                    s.IssuerURI,
		SingleSignOnURI:              s.SingleSignOnURI,
		MetadataXML:                  s.MetadataXML,
		RootURIVersion:               model.SAMLRootURIVersion(s.RootURIVersion),
		PrincipalAttributeMappings:   s.PrincipalAttributeMappings,
		ServiceProviderIssuerURI:     s.ServiceProviderIssuerURI,
		ServiceProviderInitiationURI: s.ServiceProviderInitiationURI,
		ServiceProviderMetadataURI:   s.ServiceProviderMetadataURI,
		ServiceProviderACSURI:        s.ServiceProviderACSURI,
		SSOProviderID:                s.SSOProviderID.Int32,
		Serial:                       s.Serial.ConvertToAppModel(),
	}
}
...
func (s *SSOProvider) ConvertToAppModel() *model.SSOProvider {
	return &model.SSOProvider{
		Type:         model.SessionAuthProvider(s.Type),
		Name:         s.Name,
		Slug:         s.Slug,
		OIDCProvider: s.OIDCProvider.ConvertToAppModel(),
		SAMLProvider: s.SAMLProvider.ConvertToAppModel(),
		Serial:       s.Serial.ConvertToAppModel(),
	}
}
...
func (s User) ConvertToAppModel() model.User {
	return model.User{
		SSOProvider:   s.SSOProvider.ConvertToAppModel(),
		SSOProviderID: s.SSOProviderID.Int32,
		AuthSecret:    s.AuthSecret.ConvertToAppModel(),
		AuthTokens:    s.AuthTokens.ConvertToAppModel(),
		Roles:         s.Roles.ConvertToAppModel(),
		FirstName:     s.FirstName.String,
		LastName:      s.LastName.String,
