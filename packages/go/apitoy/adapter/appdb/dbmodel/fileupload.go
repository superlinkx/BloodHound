...
func (s FileUploadJob) ConvertToAppModel() model.FileUploadJob {
	return model.FileUploadJob{
		UserID:           s.UserID,
		UserEmailAddress: s.UserEmailAddress.String,
		User:             s.User.ConvertToAppModel(),
		Status:           model.JobStatus(s.Status),
